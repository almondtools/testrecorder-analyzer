...
import static net.amygdalum.testrecorder.analyzer.query.ContextSnapshotPredicate.onExpectArg;
import static net.amygdalum.testrecorder.analyzer.query.ContextSnapshotPredicate.onExpectThis;
...
public class TestDatabaseQueryTest {
	...
	void testSelectionOnSetupThis() throws Exception {
		List<TestCase> cases = database.fetch(query(onContextSnapshot(onSetupThis(value -> new SerializedValueWalker(value)
				.field("attribute")
				.forNull(v -> true)
				.andRecover(false)))))
			.collect(toList());
	...
	void testSelectionOnExpectThis() throws Exception {
		List<TestCase> cases = database.fetch(query(onContextSnapshot(onExpectThis(value -> new SerializedValueWalker(value)
				.field("attribute")
	...
	void testSelectionOnSetupArgument() throws Exception {
		List<TestCase> cases = database.fetch(query(onContextSnapshot(onSetupArg(0, value -> new SerializedValueWalker(value)
	...
	void testSelectionOnExpectArgument() throws Exception {
		List<TestCase> cases = database.fetch(query(onContextSnapshot(onExpectArg(0, value -> new SerializedValueWalker(value)
				.forLiteral(v -> v.getValue().equals(43))
				.andRecover(false)))))
			.collect(toList());
	...
	void testSelectionOnExpectArgumentChanging() throws Exception {
		List<TestCase> cases = database.fetch(query(onContextSnapshot(onExpectArg(0, value -> new SerializedValueWalker(value)
				.forObject(v -> v.getType() == GenericCycle.class)
				.andRecover(false)))
					.and(onContextSnapshot(onExpectArg(0, value -> new SerializedValueWalker(value)
						.field("next")
						.field("a")
	...
	void testSelectionOnSetupExpectGlobals() throws Exception {
		List<TestCase> cases = database.fetch(query(onContextSnapshot(snapshot -> {
				try {
					SerializedValue globalBeforeField = Arrays.stream(snapshot.getSetupGlobals())
						.filter(field -> field.getName().equals("global"))
						.findFirst()
						.orElseThrow(RuntimeException::new)
						.getValue();
					SerializedValue globalAfterField = Arrays.stream(snapshot.getExpectGlobals())
						.filter(field -> field.getName().equals("global"))
						.findFirst()
						.orElseThrow(RuntimeException::new)
						.getValue();
					Object globalBefore = globalBeforeField instanceof SerializedLiteral ? ((SerializedLiteral) globalBeforeField).getValue() : null;
					Object globalAfter = globalAfterField instanceof SerializedLiteral ? ((SerializedLiteral) globalAfterField).getValue() : null;
	...
	void testSelectionOnSetupInput() throws Exception {
		List<TestCase> cases = database.fetch(query(onContextSnapshot(snapshot -> {
				return !snapshot.getSetupInput().isEmpty();
			})))
			.collect(toList());
	...
	void testSelectionOnExpectOutput() throws Exception {
		List<TestCase> cases = database.fetch(query(onContextSnapshot(snapshot -> {
				return !snapshot.getExpectOutput().isEmpty();
			})))
			.collect(toList());
